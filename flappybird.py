...
def determine_action(bird_v_distance, bird_h_distance, alive, dict):
    state_click = dict.get((bird_v_distance, bird_h_distance, alive, True), 0)
    state_no_click = dict.get((bird_v_distance, bird_h_distance, alive, False), 0)
    ...
    if (state_click <= state_no_click):
        return False
    else:  # state_click > state_no_click:
        return True

        
def get_max_reward(bird_v_distance, bird_h_distance, alive, dict):

    state_click = dict.get((bird_v_distance, bird_h_distance, alive, True), 0)
    state_no_click = dict.get((bird_v_distance, bird_h_distance, alive, False), 0)
    
    if((state_click <= state_no_click)):
        return state_no_click
    else:
        return state_click


def update_state(state, previous_state, dict, alive, max_reward):
    if alive == False:
        reward = -1000
    else:
        reward = 1
    ...
    alpha = 0.7
    discount = 1
   # max_reward = get_max_reward(bird_v_distance, bird_h_distance, alive, dict)
  #  state = (bird_v_distance, bird_h_distance, alive, action)
    #max_reward = dict.get((state), 0)
   # previous_state = (previous_v, previous_h, previous_alive, previous_action)
    old_value = dict.get((previous_state), 0)
    reward = reward
    learned_value = reward + discount*max_reward
    
    
   
    dict[previous_state] = (old_value + alpha * (learned_value - old_value))
...
def main():
    ...
    while(1): 
        ...
        #random.seed(0) # Turns off pipe spawn rng
        pygame.init()
        ...
        previous_alive = True # Should always be True, possible edge case?
        restart = False
        ...
        while not done:
            ...
            if fps_to_next_action == 0:
                print "action"
                bird_v_distance = int(bird.y - current_pipe.PIECE_HEIGHT * current_pipe.bottom_pieces )
                bird_h_distance = int(current_pipe.x - bird.x)
                
                
                action = determine_action(bird_v_distance, bird_h_distance, alive, dict)
                if action == True:
            ...
            # update learned values every 10 frames or after death
            if(fps_to_next_action >= fps_action or done == True):
                if done == True:
                    alive = False
                ...
                update_state(state, previous_state, dict, alive, max_reward)
                
                previous_v = bird_v_distance
                previous_h = bird_h_distance
            ...
            while pipes and not pipes[0].visible:
                pipes.popleft()
                current_pipe = pipes[0]
                print "popped: " + str(bird_v_distance)
                print "popped: " + str(bird_h_distance)
            ...
            for p in pipes:
                if p.x + PipePair.WIDTH < bird.x and not p.score_counted:
                    print "score: " + str(bird_h_distance)
                    print "score: " + str(bird_v_distance)
                    score += 1
